...
class PegResult:
	...
	func _init( _accept:bool = false, _length:int = 0, _capture:Array = [] ):
		self.accept = _accept
		self.length = _length
...
class PegDebug:
	...
	func parse( buffer:String, p:int ) -> PegResult:
		...
		return PegResult.new( self.accept )
...
class PegGreedy extends PegTree:
	...
	func parse( buffer:String, p:int ) -> PegResult:
		...
		return PegResult.new( self.least <= count, total_length, total_capture )
